...
- **`DocumentRow`'s collapsed-row height cache is width-tagged, and stale spacers *approximate* rather than re-measure.** The row-collapse optimization (§4) reserved each off-screen row's exact height with a spacer. After a window resize (rotation, entering Split View) those heights are wrong for the new wrap width — the moment the app entered Split View, the main thread livelocked at 100 % CPU with the scene frozen at its pre-split render. This took three attempts, each diagnosed by `sample`:
  ...
  - *Follow-up (scroll position on rotation):* the width fed to that scaling must be the readable-capped column width, not the full window. `onGeometryChange` was chained after the outer `.frame(maxWidth: .infinity)`, so it reported the whole screen. On any window wider than the 672 pt cap (all iPad orientations, large-iPhone landscape) the text column stays pinned at the cap and never reflows on rotation — yet the reported screen width still changed, so every collapsed row rescaled its spacer (e.g. ×0.75) for a reflow that never happened, mis-sizing the reserved height above the viewport and jumping the reading position on orientation change. Fix: read the geometry at the `.frame(maxWidth: readableWidth)` frame (the actual wrap width), so above the cap the value is constant across rotation and position holds; below it (narrow iPhone, ⅓ Split View) it still tracks the true reflow width.
