...
**Verification status (2026-07-14) — idle-scroll hitch gate OPEN.** Functional checks all pass (149 unit tests; the full 9-point live checklist on an iOS 18.5 sim; 73,399-match query on stress-5k with prompt navigation; CPU settles to 0.0 % after flings with and without a query — no livelock). But the §8 stress-5k autoscroll gate regressed: baseline `main` 6.41 ms/s hitch vs 146.7–157.2 ms/s on this branch (two runs), search *idle*. A/B with the visibility feed disabled measured 92.2 ms/s, so `onScrollVisibilityChange` per row is ~40 % of the cost; the prime suspect for the rest is the per-leaf search plumbing that now runs at every row materialization (`SearchArrivalFlash`'s unconditional `.task(id:)` spawns a Task per text leaf, plus two observable-environment reads per leaf body). Known smaller defect: rotation re-anchors to the last *user-scrolled* row, so rotating right after a search jump returns to the pre-jump position (`scrollTo` never writes the `scrollPosition(id:)` binding). Both must be fixed before this section's design can be called settled.
