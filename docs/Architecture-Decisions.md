...
- **Expand blocks, measured (2026-07-16).** The full interleaved gauntlet passed live on stress-5k: an expand opened at 100% stayed open and correctly scaled through size-up → rotation cycle → collapse-to-spacer (flung ~2,200 blocks away) → size-reset and rotation cycle while collapsed → return (`model.expandedBlocks` is written by no type-size code path — audited exhaustively). Two bounded residuals, verified by measurement: (1) a size tap while the viewport top is *inside* a tall open row (expand body, long code block) snaps forward to the next row's top — `scrollPosition(id:)` tracks the first row whose leading edge is below the viewport top, so the loss is capped at the tall row's visible remainder (< one viewport, non-accumulating; an audit's claim of an unbounded snap *back* to the expand header was refuted empirically). (2) The `ratio²` factor over-estimates collapsed OPEN expand bodies dominated by media/code (worst case media-gallery bodies); position still holds via the identity re-pin — only scrollbar proportions and content height are briefly wrong, corrected on natural re-entry. Pre-existing and untouched by this branch: the width-keyed memo is expand-state-blind (a sample recorded closed at one width replays as exact when the row re-enters open at that width — one-time, screens-sized jump for large bodies; candidate follow-up: reset non-current-width samples on expand toggle).
